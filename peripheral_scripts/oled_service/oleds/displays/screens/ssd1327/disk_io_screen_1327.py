...
class DiskIOScreen1327(BaseScreen):
    ...
    def __init__(self):
        ...
        self._scale_bps = 1.0
        try:
            self._alpha = float(os.getenv("OLED_IO_EMA_ALPHA", "0.3"))
        except Exception:
            self._alpha = 0.3
        try:
            self._decay = float(os.getenv("OLED_IO_TREND_DECAY", "0.90"))
        except Exception:
            self._decay = 0.90
    ...
    def draw(self, dm, stats):
        ...
        row = 0
        row = grid.text_row(cv, dm, row, "Disk I/O", font=dm.font_small, fill=c)

        line = f"R {self._fmt_bps(r_bps)}   W {self._fmt_bps(w_bps)}"
        row = grid.text_row(cv, dm, row, line, font=dm.font, fill=c)
