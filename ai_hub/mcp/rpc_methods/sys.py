...
from functions.sys.model import DigestSummary, UnitReport, Incident
from mcp.logic import sys_rules, sys_templates # Предполагаем, что логика вынесена

async def build_digest(app: AppContext, config_name: str) -> None:
    """Собирает полный дайджест состояния системы."""
    print(f"Executing job: sys.build_digest for config '{config_name}'")
    ...
    digest = DigestSummary(
        lookback=cfg.lookback, min_priority=cfg.min_priority, overall=overall_level, reports=reports
    )
