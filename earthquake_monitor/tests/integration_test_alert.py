# Файл: src/test_alert.py

import time
from unittest.mock import MagicMock

# Импортируем все ваши реальные компоненты
...
def create_fake_earthquake(mag, event_id="fake_event_1"):
    """Создает поддельный ответ API с землетрясением нужной магнитуды."""
    return {
        "features": [
...
def run_physical_test():
    ...
    # 1. Создаем поддельный источник данных
    # Он вернет сильное землетрясение, когда его попросят
    ...
    # 2. Создаем РЕАЛЬНЫЙ звуковой оповещатель
    # Он будет отправлять настоящие команды в сокет
    real_sound_alerter = SoundAlerter()
    log.info("Real SoundAlerter created.")

    # 3. Собираем монитор с нашими компонентами
